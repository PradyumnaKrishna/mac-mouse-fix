    ...
    func testTakeScreenshots_Documentation() throws { // TODO: Rename to testTakeScreenshots_ForGuides
        ...
        var args = Args(
            ...
            allFilePaths: [
                ...
                "Markdown/Media/Autogenerated/%@/CapturedButtons0.jpg", /// The `%@` is to be replaced with the locale code.
                "Markdown/Media/Autogenerated/%@/CapturedButtons1.jpg", /// Putting everything in an Autogenerated folder, so we don't accidentally delete the manually added stuff. [Sep 2025]
                "Markdown/Media/Autogenerated/%@/CapturedButtons2.jpg",
            ],
        );
        if (args.onlyUpdateLocales.count != 1) {
            args.killApp = true                     /// (Need to restart app between locale-switches if there are multiple locales)
        }
        ...
        do {
            ...
            let configURL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appending(path: "/in.onpy.mac-mouse-fix/config.plist")
            ...
            do {
                ...
                do {
                    ...
                    for i in 1...2 {
                        ...
                        writeImageToFile(resultImage, absoluteURLFromRepoRelativePath(args.allFilePaths[i], locale), [.compressionFactor : 0.0]) /// Lowest quality. TODO: Optimize. Idea: Downscale and compress less? || Also remember the .writeImageToFile call above
                    }
                    NSWorkspace.shared.activateFileViewerSelecting(args.allFilePaths.map { absoluteURLFromRepoRelativePath($0, locale) })
                }
            }
        }
    ...
    func sharedf_launch_app(url: URL, args: [String], env: [String: String], kill_app: Bool) -> XCUIApplication {
        ...
        if (!kill_app) {
            ...
            if (url == sharedf_helper_url()) {
                sharedf_clt("launchctl remove in.onpy.mac-mouse-fix.helper") /// Prevent launchd from restarting the helper
            }
