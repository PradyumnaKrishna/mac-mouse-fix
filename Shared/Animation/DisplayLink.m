...
- (instancetype)initOptimizedForWorkType:(MFDisplayLinkWorkType)workType {
    ...
    if (self) {
        ...
        _displayLinkQueue = dispatch_queue_create("in.onpy.mac-mouse-fix.helper.display-link", attrs); /// TODO: Remove .helper from the queue name. This is used in the mainApp, too.
...
- (void)setUpNewDisplayLinkWithActiveDisplays {
    ...
    /// Validate thread
    {
        ...
        ///         - `[GestureScrollSimulator initialize]` calls this on `in.onpy.mac-mouse-fix.helper.display-link` queue,
        ///         - `[Scroll load_Manual]` calls this on mainThread.
        ///         - `[ModifiedDragOutputTwoFingerSwipe load_Manual]` calls this on the mainThread.
        ///         - If the displayLink is recreated after detaching/reattaching a display, it seems to always run on `in.onpy.mac-mouse-fix.helper.display-link` (Haven't done too much testing or thinking here.)
