...
class ReplaceAnimations {
    ...
    private static func fadeInDelayDispatchQueue(forView view: NSView) -> DispatchQueue {
        if let cachedQueue = _fadeInDelayDispatchQueues[view] {
            ...
            let newQueue = DispatchQueue.init(label: "in.onpy.mac-mouse-fix.fadeInDelay.\(view.hash)", qos: .userInteractive, attributes: [], autoreleaseFrequency: .inherit, target: nil)
